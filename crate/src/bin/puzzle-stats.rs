...
fn main() {
    ...
    let start_date = args.start_date.parse::<NaiveDate>().expect("Invalid start date format. Use YYYY-MM-DD");
    ...
    println!("Generating stats for {} days, {} puzzles per day starting on {}...", args.days, args.puzzles_per_day, start_date);

    for day_offset in 0..args.days {
        let current_date = start_date.checked_add_days(Days::new(day_offset as u64)).unwrap();
        ...
        for i in 1..=args.puzzles_per_day {
            ...
            *complexity_distribution.entry(rating.complexity).or_insert(0) += 1;
    ...
    for (sols, count) in &solutions_distribution {
        println!("  {} solutions: {} ({:.1}%)", sols, count, (*count as f64 / total_puzzles as f64) * 100.0);
    }
    ...
    println!("  Variance explained by Day (R^2): {:.2}%", r_squared * 100.0);
    if r_squared > 0.1 {
